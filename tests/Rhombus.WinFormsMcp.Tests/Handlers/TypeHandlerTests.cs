using System.Text.Json;
using C5T8fBtWY.WinFormsMcp.Server.Handlers;
using C5T8fBtWY.WinFormsMcp.Tests.Mocks;
using NUnit.Framework;

namespace C5T8fBtWY.WinFormsMcp.Tests.Handlers;

[TestFixture]
public class TypeHandlerTests
{
    private MockSessionManager _session = null!;
    private MockWindowManager _windows = null!;
    private TypeHandler _handler = null!;

    [SetUp]
    public void Setup()
    {
        _session = new MockSessionManager();
        _windows = new MockWindowManager();
        _windows.AddWindow("Test Window", "0x1234", 1234);
        _handler = new TypeHandler(_session, _windows);
    }

    [Test]
    public void SupportedTools_ReturnsType()
    {
        Assert.That(_handler.SupportedTools, Contains.Item("type"));
    }

    [Test]
    public async Task ExecuteAsync_MissingText_ReturnsError()
    {
        var args = JsonDocument.Parse("{}").RootElement;

        var result = await _handler.ExecuteAsync("type", args);

        Assert.That(result.GetProperty("success").GetBoolean(), Is.False);
        Assert.That(result.GetProperty("error").GetString(), Does.Contain("text is required"));
    }

    [Test]
    public async Task ExecuteAsync_EmptyText_ReturnsError()
    {
        var args = JsonDocument.Parse("{\"text\": \"\"}").RootElement;

        var result = await _handler.ExecuteAsync("type", args);

        Assert.That(result.GetProperty("success").GetBoolean(), Is.False);
        Assert.That(result.GetProperty("error").GetString(), Does.Contain("text is required"));
    }

    [Test]
    public async Task ExecuteAsync_ElementNotFound_ReturnsError()
    {
        var args = JsonDocument.Parse("{\"text\": \"Hello\", \"target\": \"elem_999\"}").RootElement;

        var result = await _handler.ExecuteAsync("type", args);

        Assert.That(result.GetProperty("success").GetBoolean(), Is.False);
        Assert.That(result.GetProperty("error").GetString(), Does.Contain("Element not found"));
    }

    // Note: StaleElement test requires real AutomationElement objects
    // which can't be easily mocked. This is tested in integration tests.

    [Test]
    public async Task ExecuteAsync_ResponseIncludesWindows()
    {
        // Even errors should include windows for context
        var args = JsonDocument.Parse("{}").RootElement;

        var result = await _handler.ExecuteAsync("type", args);

        Assert.That(result.TryGetProperty("windows", out var windows), Is.True);
        Assert.That(windows.GetArrayLength(), Is.EqualTo(1));
    }
}
